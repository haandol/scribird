...
final class CaptureSourceMappingTests: XCTestCase {
    ...
    func test_captureLabel_namesTheDevicePathNotTheSpeaker() {
        XCTAssertEqual(Speaker.me.captureLabel, "마이크")
        XCTAssertEqual(Speaker.remote.captureLabel, "시스템 오디오")
        XCTAssertNotEqual(Speaker.me.captureLabel, Speaker.me.displayName,
                          "장치 경로 이름과 화자 이름이 같으면 경고 문구가 대상을 가리키지 못한다")
    }
    ...
    func test_captureLabel_agreesBetweenSpeakerAndChange() {
        for change in [AudioDeviceMonitor.Change.input, .output] {
            XCTAssertEqual(change.captureLabel, change.speaker.captureLabel)
        }
    }
}
