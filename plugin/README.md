...
오디오 파일명, 세션 디렉터리 규칙)가 앱과 함께 바뀌기 때문이다. 따로 두면
...
이 플러그인은 `remote.m4a`가 있는 **과거 소스 분리 세션**을 대상으로 한다. 현재 앱의
`meeting.m4a` 단일 합본은 입력 소스가 섞여 있어 이 워크플로가 받지 않는다.

과거 세션의 `remote.m4a`를 AWS Transcribe로 화자 분리해 `Remote`를 실제 참석자 이름 또는
