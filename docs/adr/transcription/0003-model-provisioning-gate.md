...
넣는 중이었다. 앱이 진행률을 정체로 판정해 설치 요청을 취소하자 `transcription.ko`
구독이 해제되고, 이미 설치돼 있던 한국어 에셋까지 현재 자산 세트에서 빠졌다.
...
- 시스템이 보고하는 진행률과 `.downloading` 상태는 실제 전송과 자산 구성·대기열 준비를
  구분하지 못한다.
- 설치 요청 취소는 단순 UI 취소가 아니라 언어 구독 해제와 설치된 에셋 회수로 이어질 수 있다.
...
모델 설치와 세션 예약은 별도 생명주기로 다룬다. 설치 경로는 로케일을 예약하지 않고
macOS 설치 요청의 완료 또는 오류를 기다린 뒤 실제 설치 목록을 확인한다. 예약은 설치된
모델을 사용하는 세션이 시작될 때만 수행한다.
...
- 앱은 진행률이나 `.downloading` 지속 시간을 근거로 설치 요청을 취소하거나 실패로
  판정하지 않는다.
- 설치 경로에서는 모델을 예약하지 않는다. 설치 요청이 완료된 뒤 실제 설치 목록에 언어가
  있어야 성공이며, 세션 시작 시에만 설치된 모델의 예약을 시도한다.
- 설치 요청이 오류를 반환하거나 완료 후 필요한 에셋이 설치 목록에 없으면 성공으로 보지
  않는다. 즉시 실패 상태와 이유를 표시하고 화면 잠금을 푼다.
...
flowchart TD
    ...
    Progress --> Complete{"시스템 요청 완료 또는 오류?"}
    Complete -- 오류 --> Error["실패 이유 표시<br/>화면 잠금 해제"]
    Complete -- 완료 --> Verify{"설치 목록에 언어가 존재?"}
    Verify -- 아니오 --> Error
    Verify -- 예 --> Ready
...
- 시스템 준비를 다운로드 정체로 오판해 언어 구독과 설치된 에셋을 스스로 해제하지 않는다.
- 설치와 예약을 분리해 미설치 언어 요청이 현재 설치된 자산 세트를 먼저 잠그지 않는다.
