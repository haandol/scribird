...
결과에는 `나`, 시스템 출력을 담당하는 분석기의 결과에는 `상대방`을 붙인다. 각
분석기는 자기 소스만 보므로 화자 라벨이 **추론 없이 확정**된다 — 신뢰도나 음성
특징을 따질 필요가 없다.

이 결정의 필연적 결과로 **화자 분리는 `나` vs `상대방 전원` 2분리가 상한**이다.
...
때문이다. 게다가 소스를 믹스하면 지금은 공짜로 얻는 `나` vs `상대방` 구분까지 추론
...
- 대면 회의에서 상대방이 내 마이크로 함께 들어오면 그 발언이 `나`로 라벨된다. 소스가
