...
동시에 이 앱의 성립 근거가 **회의 데이터 처리가 기기 안에서 끝난다**는 것이다. 회의 음성과
회의록이 외부로 나가지 않는다는 전제 위에서 사용자가 민감한 회의를 녹취한다. 필수 English
전사 모델이 없으면 macOS가 시스템 Speech 에셋을 내려받을 수 있지만, 이 요청에는 회의 데이터가
들어가지 않는다.
...
**새 버전은 사용자가 요청할 때만 조회한다.** 설정 창에서 버전 확인을 누른 그 순간에 한 번
조회하고, 앱 시작 시 조회도, 주기적 조회도, 조회 여부를 켜는 설정도 두지 않는다.

필수 English Speech 에셋 설치는 이 규칙의 유일한 시작 시 예외다. 앱 시작 시 모델이 없으면
macOS 시스템 설치를 시작할 수 있다. 이 예외는 앱 코드나 업데이트를 내려받는 경로가 아니며,
회의 음성·전사·사용 통계·기기 식별자를 보내지 않는다. 한국어 모델은 사용자가 설정에서 설치를
요청할 때만 내려받는다.

이것이 이 결정의 핵심이다. 자동 조회를 기본 꺼진 설정으로 두는 선택도 가능했지만, 그러면
앱이 임의로 외부 릴리즈 서버에 접속하는지를 설정에서 확인해야 한다. 버전 확인은 사용자가
누를 때만 실행하고, 시스템 Speech 에셋 설치와 앱 자체의 외부 조회는 데이터 경계와 목적이
다르므로 구분해 설명한다.
