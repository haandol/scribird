...
### 위에 머무는 것과 앞을 가로막는 것은 다르다

**이 창이 다른 앱 위에 있어야 하는 이유는 회의 화면에 가려지지 않기 위한 것이지, 사용자가
방금 불러낸 것을 덮기 위한 것이 아니다.** 회의 앱은 사용자가 배경에 두고 참여하는 것이라
그 위에 머무는 것이 도움이 되지만, 사용자가 어떤 조작으로 직접 불러낸 창(앱이 산출물을 보여주려
연 파일 탐색기, 이 앱의 설정 창)은 그것을 보려고 부른 것이므로 덮으면 부르지 않은 것과
구분되지 않는다.

그래서 **앱이 스스로 다른 창을 앞으로 내보내는 동작을 할 때는 그 창이 전사 창보다 앞에
온다.** 산출물 폴더를 열어 놓고 그 위를 전사 창이 덮으면, 폴더를 열어 준 목적이 사라진다.

**사용자가 창을 치우는 수단은 창 자체에 있어야 하고, 그 수단은 이 플랫폼의 관례를 따른다.**
자동으로 닫히지 않는 창이므로 치우는 방법이 없으면 화면에 남는다. 닫기 버튼과 전역 단축키
외에 **창을 닫는 표준 단축키(`⌘W`)로도 닫힌다** — 사용자가 이미 아는 조작을 다시 배우게 하지
않는다. 닫는 것은 숨기는 것이며 녹취에는 영향이 없다(위의 독립 원칙).
...
- 앱이 불러낸 창이 전사 창에 덮이지 않으므로, 산출물 폴더를 열어 주는 동작이 실제로 산출물을
  보여준다.
- 창을 치우는 수단이 표준 단축키까지 셋이므로, 사용자가 아는 조작으로 즉시 정리할 수 있다.
...
- 창이 다른 앱 위에 머무르므로, 화면 공유 중에 띄우면 전사 내용이 참석자에게 노출될 수
  있다.
- 앱이 불러낸 창에 앞자리를 넘기는 동안에는 전사 창이 그 뒤로 내려가므로, 그 순간 회의 화면이
  전사 창을 덮을 수 있다. 확인용 화면이 가려지지 않는다는 성질과 사용자가 부른 것을 보여준다는
  성질이 부딪히는 지점이고, 후자를 택했다.
