...
끊기, 소스별 레벨 미터와 경고, 트랜스크립트, 현재 세션의 저장 위치. 이 목록이 곧 "회의 중
필요한 것"의 정의다. 저장 위치가 여기 있는 이유는 그것이 설정값이 아니라 진행 중인 세션의
상태이기 때문이다 — 세션마다 바뀌고, 녹취가 실제로 저장되고 있는지 확인하는 근거다.

**설정 창으로 옮기는 것**은 한 번 정하고 잊는 것이다 — 회의 언어, 음성 원본 저장 여부,
전역 단축키, 종료 시 저장 폴더를 자동으로 열지 여부. 그리고 앱 자체에 대한 정보와
조작(버전 확인, 종료)도 여기에 둔다.
...
- **기본값은 회의 언어 `한국어 + English`, 음성 원본 저장 켬, 종료 시 저장 폴더 열기
  켬이다.** 사용자가 아무것도 정하지 않은 상태에서도 두 언어가 인식되고, 원본이 남고,
...
flowchart LR
    ...
    T -->|"설정 버튼 · ⌘,"| S
    T --- TC["시작·중지<br/>세션 경계<br/>레벨 미터·경고<br/>트랜스크립트<br/>현재 언어 표시<br/>현재 저장 위치"]
    S --- SC["회의 언어<br/>음성 원본 저장<br/>종료 시 저장 폴더 열기<br/>전역 단축키<br/>버전 확인<br/>종료"]
```
...
- 설정이 유지되므로 사용자가 지난번에 정한 값을 잊고 녹취를 시작할 수 있다. 원본 저장을 껐던
  것을 잊으면 남기려던 음성이 없고, 단일 언어로 뒀던 것을 잊으면 다른 언어 발화가 인식되지
  않는다. 그래서 **현재 언어 구성은 전사 화면에서 읽을 수 있어야 한다**는 위 결정이 유지에
  대한 완화 장치로도 작동한다.
