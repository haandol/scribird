...
것"이므로, 유지되지 않는 항목은 그 정의를 만족하지 못한다. 회의 언어와 음성 원본 저장
여부도 전역 단축키와 같은 계약을 따른다 — 세 항목 사이에 예외를 두지 않는다.

- **기본값은 회의 언어 `한국어 + English`, 음성 원본 저장 켬이다.** 사용자가 아무것도
  정하지 않은 상태에서도 두 언어가 인식되고 원본이 남아야 한다. 저장된 값이 없을 때만
  기본값이 쓰이고, 사용자가 정한 값은 그 값이 계약이 된다.
...
- **녹취 중에 바뀐 값은 없다** — 두 항목 모두 녹취 중 잠기므로, 저장은 사용자가 바꾼
  시점에만 일어난다.
