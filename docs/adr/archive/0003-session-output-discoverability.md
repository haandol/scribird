...
산출물을 어디에 둘지 사용자가 정하면 자기가 정한 곳이므로 찾지 못할 일이 없다. 클라우드
동기화 폴더에 두는 등의 활용도 열린다.

채택하지 않은 이유는 이 문제와 다른 문제를 푼다는 것이다. 사용자가 못 찾는 이유는 위치가
문서 폴더 아래인 것이 아니라 **회의별 디렉터리 이름이 화면에 없기** 때문이고, 위치를 고르게
해도 어느 회의가 어느 디렉터리인지는 여전히 보이지 않는다. 게다가 선택 가능한 위치는 접근
권한·이동·부재를 다뤄야 하는데, 회의 중에 그 실패가 드러나면 녹취를 잃는다. 위치 선택은
별개의 결정으로 다룰 문제다.
